...
**The canonical data model drives the normalization.** `etl/dbt/models/staging/schema.yml` declares one entity per route — columns, `data_type`, cleanup expressions (`meta.expr`), and data tests. From that single file: the staging SQL is *generated* (`build_staging` macro — `cast` from `data_type`, cleanup from `meta.expr`), the ingest landing gate (`require_columns`) is *derived*, and the tests are *enforced* on every build (a violation quarantines the file). Route specs only carry what's genuinely per-vendor: `column_aliases` mapping name variants (`Seg`, `RGN`, `adr`…) onto the canonical names. Adding a field to an entity = one schema.yml block.
